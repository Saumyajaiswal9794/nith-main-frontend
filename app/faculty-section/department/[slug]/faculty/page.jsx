...
export default function FacultyPage() {
  ...
  // Group by group_title_en
  const groups = {}
  faculty.forEach(f => {
    const key = language === 'hi' && f.group_title_hi ? f.group_title_hi : (f.group_title_en || 'Faculty')
      ...
      <div style={mainStyle}>
        <div style={contentBox}>
          ...
          {Object.keys(groups).length === 0 && (
            <p style={{ fontSize: '14px', color: '#999' }}>{language === 'hi' ? 'अभी तक जोड़ा नहीं गया' : 'No faculty added yet'}</p>
          )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f5f5f5' }}>
                    ...
                    <th style={thStyle}>{language === 'hi' ? 'पदनाम' : 'Designation'}</th>
                ...
                <tbody>
                  {members.map((m, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={tdStyle}>{language === 'hi' && m.name_hi ? m.name_hi : m.name_en}</td>
                      <td style={tdStyle}>{language === 'hi' && m.designation_hi ? m.designation_hi : m.designation_en}</td>
                      <td style={tdStyle}>{language === 'hi' && m.interests_hi ? m.interests_hi : m.interests_en}</td>
